...
std::string EcobiciAPIClient::FetchGBFSFeed(const std::string &feed_name) {
    httplib::SSLClient cli("gbfs.mex.lyftbikes.com");
...
std::string EcobiciAPIClient::FetchHistoricalCSV(int year, int month) {
    httplib::SSLClient cli("ecobici.cdmx.gob.mx");
    cli.set_follow_location(true);
    cli.set_connection_timeout(10, 0);
    cli.set_read_timeout(60, 0);
    
    std::ostringstream path_stream;
    path_stream << "/wp-content/uploads/" 
                << year << "/" 
    ...
    if (!res) {
        throw IOException("Failed to fetch historical CSV for " + std::to_string(year) + "-" + 
                         std::to_string(month) + " - Connection error");
    }
    
    if (res->status == 404) {
        std::ostringstream alt_path_stream;
        alt_path_stream << "/wp-content/uploads/" 
                       << year << "/" 
                       << std::setfill('0') << std::setw(2) << month << "/"
                       << year << "-" 
                       << std::setfill('0') << std::setw(2) << month << ".csv";
        ...
        throw IOException("Failed to fetch historical CSV for " + std::to_string(year) + "-" + 
                         std::to_string(month) + " - HTTP " + std::to_string(res->status));
    }
    
    return res->body;
}

std::vector<std::string> EcobiciAPIClient::FetchHistoricalCSVRange(int start_year, int start_month, 
